...
def train_dgd_mrna(dgd, train_loader, validation_loader, device,
    ...
    for epoch in tqdm(range(nepochs)):
        ...
        for (mrna_data, lib_mrna, index) in train_loader:
            ...
            loss_tab["train_gmm"][-1] += gmm_loss.item()
            loss_tab["train_loss"][-1] += recon_loss.item() + gmm_loss.item()
        ...
        for (mrna_data, lib_mrna, index) in validation_loader:
            ...
            loss_tab["test_gmm"][-1] += gmm_loss.item()
            loss_tab["test_loss"][-1] += recon_loss.item() + gmm_loss.item()
...
def train_dgd_mirna(dgd, train_loader, validation_loader, device,
                learning_rates={'dec':0.001,'rep':0.01,'gmm':0.01}, 
                weight_decay=0., betas=(0.9, 0.999), nepochs=100, fold=None, pr=1, plot=10, 
                reduction_type="sum", scaling_type="mean",
                sample_index=[0,11,22,33], subset=1310, wandb_log=False, early_stopping=50, is_plot=False):
    # Normalization factor
    if reduction_type == "sum":
        tlen=len(train_loader.dataset)*dgd.decoder.n_out_features
        vlen=len(validation_loader.dataset)*dgd.decoder.n_out_features
    ...
    for epoch in tqdm(range(nepochs)):
        ...
        with torch.inference_mode():
            ...
            if scaling_type == "mean":
                ...
                scaling = torch.max(train_loader.dataset.data, axis=1).values
            elif scaling_type == "sum":
                scaling = torch.sum(train_loader.dataset.data, axis=1)
            ...
            mse = mean_squared_error(recon, data)
            loss_tab["train_mse"][-1] += mse.item()
            
            mae = mean_absolute_error(recon, data)
            loss_tab["train_mae"][-1] += mae.item()
            
            r2 = r2_score(recon, data)
            loss_tab["train_r2"][-1] += r2.item()
            
            spearman = spearman_corrcoef(recon, data)
            loss_tab["train_spearman"][-1] += spearman.item()
            
            pearson = pearson_corrcoef(recon, data)
            loss_tab["train_pearson"][-1] += pearson.item()
            
            msle = mean_squared_log_error(recon, data)
            loss_tab["train_msle"][-1] += msle.item()

        loss_tab["train_recon"][-1] /= tlen
        loss_tab["train_gmm"][-1] /= tlen_gmm
        loss_tab["train_loss"][-1] = loss_tab["train_recon"][-1] + loss_tab["train_gmm"][-1]
        ...
        with torch.inference_mode():
            ...
            if scaling_type == "mean":
                ...
                scaling = torch.max(validation_loader.dataset.data, axis=1).values
            elif scaling_type == "sum":
                scaling = torch.sum(validation_loader.dataset.data, axis=1)
        ...
        loss_tab["test_recon"][-1] /= vlen
        loss_tab["test_gmm"][-1] /= vlen_gmm
        loss_tab["test_loss"][-1] = loss_tab["test_recon"][-1] + loss_tab["test_gmm"][-1]
        ...
        if is_plot:
            if plot>=0 and (epoch)%plot==0:
                plot_latent_space(*dgd.get_latent_space_values("train", 3000), train_loader.dataset.label, color_mapping, epoch, dataset="Train")
                plot_latent_space(*dgd.get_latent_space_values("val", 3000), validation_loader.dataset.label, color_mapping, epoch, dataset="Validation")
                plot_mirna_recons(dgd, train_loader, sample_index, epoch, device, type="Train", scaling_type=scaling_type)
                plot_mirna_recons(dgd, validation_loader, sample_index, epoch, device, type="Validation", scaling_type=scaling_type)
        ...
        if wandb_log: 
            wandb.log({
                ...
                "epoch": epoch,
                "train_loss": loss_tab["train_loss"][-1],
                "train_recon": loss_tab["train_recon"][-1],
                "train_gmm": loss_tab["train_gmm"][-1],
                "train_mse": loss_tab["train_mse"][-1],
                "train_mae": loss_tab["train_mae"][-1],
                "train_r2": loss_tab["train_r2"][-1],
        ...
        if epoch > 300:
            if early_stopping:
                if loss_tab['test_loss'][-1] < best_loss:
                    best_loss = loss_tab['test_loss'][-1]
                    best_epoch = epoch
                    checkpoint = dgd
                elif epoch - best_epoch > early_stopping:
                    print("Early stopped training at epoch %d" % epoch)
